...
import { camelCase } from '../../../src/uni_modules/ht-ui-uni/components/common/util'
...
export function MarkdownTransform(): Plugin {
  return {
    ...
    async transform(code, id) {
      ...
      const GITHUB_URL = 'https://github.com/Moonofweisheng/ht-ui-uni/tree/master'
      const componentId = path.basename(id, '.md')
      const componentName = `ht-${componentId}`
      const camelComponentId = camelCase(componentId)
      const demoUrl = `${GITHUB_URL}/src/pages/${camelComponentId}`
      const componentUrl = `${GITHUB_URL}/src/uni_modules/ht-ui-uni/components/${componentName}`
