...
<ht-drop-menu>
  ...
  <ht-drop-menu-item title="筛选" ref="dropMenu" @opened="handleOpened">
    <view>
      <ht-slider v-model="sliderValue" ref="slider" />
      <ht-cell title="标题文字" value="内容" />
      <ht-cell title="标题文字" label="描述信息" value="内容" />
      <ht-button block size="large" suck @click="confirm">主要按钮</ht-button>
    </view>
  </ht-drop-menu-item>
</ht-drop-menu>
```
...
<view style="display: flex; background: #fff; text-align: center;">
  ...
  <view style="flex: 1;">
    <ht-sort-button v-model="value2" title="上架时间" @change="handleChange2" />
  </view>
</view>
```
...
可以通过 `icon` 设置菜单右侧图标，等同于 `<ht-icon />` 的 `name` 属性。通过 `icon-size` 设置图标尺寸，等同于 `<ht-icon />` 的 `size` 属性。

```html
<ht-drop-menu>
  <ht-drop-menu-item title="地图" icon="location" icon-size="24px" />
</ht-drop-menu>
```
...
| icon-name     | 选中的图标名称(可选名称在 ht-icon 组件中)                              | string                        | -      | check      | -        |
| title         | 菜单标题                                                               | string                        | -      | -          | -        |
