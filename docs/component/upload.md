...
```html
<ht-upload :file-list="files" :action="host" :build-form-data="buildFormData" @change="handleChange"></ht-upload>
```
...
```html
<ht-upload :file-list="fileList" action="https://mockapi.eolink.com/zhTuw2P8c29bc981a741931bdd86eb04dc1e8fd64865cb5/upload" @change="handleChange">
    <ht-button>上传</ht-button>
</ht-upload>
```
...
```html
<ht-upload accept="video" multiple :file-list="fileList" :action="action" @change="handleChange"></ht-upload>
```
...
```html
<ht-upload accept="media" multiple :file-list="fileList" :action="action" @change="handleChange"></ht-upload>
```
...
```html
<ht-upload accept="file" multiple :file-list="fileList" :action="action" @change="handleChange"></ht-upload>
```
...
```html
<ht-upload accept="all" multiple :file-list="fileList" :action="action" @change="handleChange"></ht-upload>
```
...
<ht-upload
  ...
  @change="handleChange"
></ht-upload>
<ht-button @click="onUploadClick()">开始上传</ht-button>
```
...
```html
<ht-upload v-model:file-list="fileList" :upload-method="customUpload"></ht-upload>
```

```typescript
import type { UploadMethod, UploadFile } from '@/uni_modules/ht-ui-uni/components/ht-upload/types'
...
```html
<ht-upload v-model:file-list="fileList" accept="image" image-mode="aspectFill" :action="action">
...
| extension | 根据文件拓展名过滤(H5支持全部类型过滤,微信小程序支持all和file时过滤,其余平台不支持) | string[] | - | - | 1.0.0 |
