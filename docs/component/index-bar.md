...
使用一个固定高度的元素包裹`ht-index-bar`组件,组件的宽高会和包裹元素相同。

将`ht-index-anchor`作为子组件使用,会根据 anchor 组件的`index`属性生成锚点以及侧边栏。
...
```html [vue]
<template>
    <ht-search hide-cancel placeholder="我要去哪里？" v-model="keyword" @search="handleSearch" @clear="handleClear" />
    <view class="wraper">
