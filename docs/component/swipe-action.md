...
`ht-swipe-action`分为三部分：`自定义左按钮内容`、`自定义内容`、`自定义右按钮内容`。自定义按钮内容需要设置`slot`开启，自定义内容为默认插槽，无需手动开启。
...
> `ht-swipe-action`组件提供`left`/`right`两个滑动按钮，通过设置插槽`left`和`right`开启
...
<view class="button-group">
  <ht-button @click="changeState('left')">打开左边</ht-button>
  <ht-button @click="changeState('close')">关闭所有</ht-button>
  <ht-button @click="changeState('right')">打开右边</ht-button>
</view>
```
