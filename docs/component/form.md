...
结合 `ht-form` 组件，可以实现对以上组件的规则校验。

> 对于表单组件，建议对 ht-cell-group 开启 border 属性，这样每条 cell 就会有边框线隔离开，这样表单的划分比较清晰。
...
<ht-form ref="form" :model="model">
  ...
  <view class="footer">
    <ht-button type="primary" size="large" @click="handleSubmit" block>提交</ht-button>
  </view>
</ht-form>
```
...
<ht-form ref="form" :model="model">
  <ht-cell-group border>
    ...
    <ht-cell title-width="0px">
      <view class="footer">
        <ht-button size="small" type="info" plain @click="addPhone">添加</ht-button>
        <ht-button size="small" type="info" plain @click="removePhone">删除</ht-button>
        <ht-button size="small" type="info" plain @click="reset">重置</ht-button>
        <ht-button type="primary" size="small" @click="submit">提交</ht-button>
      </view>
    </ht-cell>
  </ht-cell-group>
</ht-form>
...
<ht-form ref="form" :model="model" errorType="toast">
  ...
  <view class="footer">
    <ht-button type="primary" size="large" @click="handleSubmit" block>提交</ht-button>
    <ht-button type="primary" size="large" @click="handleValidate" block>校验用户名和密码</ht-button>
  </view>
</ht-form>
```
...
<view>
  <ht-message-box />
  <ht-toast />
  <ht-form ref="form" :model="model" :rules="rules">
    <ht-cell-group custom-class="group" title="基础信息" border>
      <ht-input
        label="优惠券名称"
        label-width="100px"
        :maxlength="20"
      ...
      />
      <ht-cell prop="threshold" title="券面额" required title-width="100px" custom-value-class="cell-left">
    ...
    <ht-cell-group custom-class="group" title="时间和地址" border>
      <ht-datetime-picker label="时间" label-width="100px" placeholder="请选择时间" prop="time" v-model="model.time" />
      <ht-calendar label="日期" label-width="100px" placeholder="请选择日期" prop="date" v-model="model.date" />
    ...
    <ht-cell-group custom-class="group" title="其他信息" border>
      <ht-textarea
        label="活动细则"
        label-width="100px"
        type="textarea"
        ...
        prop="content"
      />
      <ht-cell title="发货数量" title-width="100px" prop="count">
        <view style="text-align: left">
          <ht-input-number v-model="model.count" />
        </view>
      </ht-cell>
      <ht-cell title="开启折扣" title-width="100px" prop="switchVal" center>
        <view style="text-align: left">
          <ht-switch v-model="model.switchVal" />
        </view>
      </ht-cell>
      <ht-input
        label="歪比巴卜"
      ...
      />
      <ht-input label="玛卡巴卡" label-width="100px" prop="phone" placeholder="请输入玛卡巴卡" clearable v-model="model.phone" />
      <ht-cell title="活动图片" title-width="100px" prop="fileList">
        <ht-upload :file-list="model.fileList" action="https://ftf.jd.com/api/uploadImg" @change="handleFileChange"></ht-upload>
      </ht-cell>
    </ht-cell-group>
    <view class="tip">
      <ht-checkbox v-model="model.read" prop="read" custom-label-class="label-class">
        已阅读并同意
...
<script lang="ts" setup>
import { useToast } from '@/uni_modules/ht-ui-uni'
import { isArray } from '@/uni_modules/ht-ui-uni/components/common/util'
import { FormRules } from '@/uni_modules/ht-ui-uni/components/ht-form/types'
import { reactive, ref } from 'vue'
