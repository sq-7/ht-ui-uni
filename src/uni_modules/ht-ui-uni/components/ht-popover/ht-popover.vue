<template>
  <view :class="`ht-popover ${customClass}`" :style="customStyle" id="popover" @click.stop="popover.noop">
    <!-- 使用插槽时无法获取正确宽高 -->
    <view class="ht-popover__pos ht-popover__hidden" id="pos">
      <view :class="`ht-popover__container ${customPop}`">
        <view v-if="!useContentSlot && mode === 'normal'" class="ht-popover__inner">
          {{ content }}
        </view>
        <view v-if="!useContentSlot && mode === 'menu' && typeof content === 'object'" class="ht-popover__menu">
          <view v-for="(item, index) in content" :key="index" class="ht-popover__menu-inner" @click="menuClick(index)">
            <ht-icon v-if="item.iconClass" :name="item.iconClass" custom-class="ht-popover__icon" />
            <text>{{ item.content }}</text>
          </view>
        </view>
      </view>
    </view>
    <ht-transition custom-class="ht-popover__pos" :custom-style="popover.popStyle.value" :show="showPopover" name="fade" :duration="200">
      <view :class="`ht-popover__container ${customPop}`">
        <view
          v-if="props.visibleArrow"
          :class="`ht-popover__arrow ${popover.arrowClass.value} ${customArrow}`"
        ...
        <!-- 列表模式 -->
        <view v-if="!useContentSlot && mode === 'menu'" class="ht-popover__menu">
          <view
            v-for="(item, index) in content"
            :key="index"
            class="ht-popover__menu-inner"
            @click="menuClick(index)"
            :style="index === 0 ? 'border-top: none' : ''"
          >
            <ht-icon v-if="typeof item === 'object' && item.iconClass" :name="item.iconClass" custom-class="ht-popover__icon" />
      ...
      <ht-icon v-if="showClose" name="close" custom-class="ht-popover__close-icon" @click="toggle"></ht-icon>
    </ht-transition>
    <view @click="toggle" class="ht-popover__target" id="target">
      <slot />
    </view>
  </view>
</template>
