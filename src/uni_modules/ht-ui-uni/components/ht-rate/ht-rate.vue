<template>
  <view :class="`ht-rate ${customClass}`" :style="customStyle" @touchmove="onTouchMove">
    <view
    ...
    >
      <ht-icon
        custom-class="ht-rate__item-star"
        :name="isActive(rate) ? activeIcon : icon"
        :size="size"
        :custom-style="rate === '100%' ? iconActiveStyle : iconStyle"
        @click="changeRate(index, false)"
      />
      <view v-if="props.allowHalf" class="ht-rate__item-half" @click.stop="changeRate(index, true)">
        <ht-icon
          custom-class="ht-rate__item-star"
          :name="isActive(rate) ? activeIcon : icon"
          :size="size"
...
function computeRateList() {
  ...
  if (typeof modelValue !== 'number') {
    console.error('[Wot Design] error(ht-rate): the value of ht-rate should be a number')
    return
  }
...
async function onTouchMove(event: TouchEvent) {
  ...
  const rateItems = await getRect('.ht-rate__item', true, proxy)
