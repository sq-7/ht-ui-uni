<template>
  <view :class="`ht-calendar ${cell.border.value ? 'is-border' : ''} ${customClass}`">
    <view class="ht-calendar__field" @click="open" v-if="withCell">
      <slot v-if="$slots.default"></slot>
      <view
        v-else
        :class="`ht-calendar__cell ${disabled ? 'is-disabled' : ''} ${props.readonly ? 'is-readonly' : ''} ${alignRight ? 'is-align-right' : ''} ${
          error ? 'is-error' : ''
        } ${size ? 'is-' + size : ''} ${center ? 'is-center' : ''}`"
      >
        <view
          v-if="label || $slots.label"
          :class="`ht-calendar__label ${isRequired ? 'is-required' : ''} ${customLabelClass}`"
          :style="labelWidth ? 'min-width:' + labelWidth + ';max-width:' + labelWidth + ';' : ''"
        >
          <slot name="label">{{ label }}</slot>
        </view>
        <view class="ht-calendar__body">
          <view class="ht-calendar__value-wraper">
            <view
              :class="`ht-calendar__value ${ellipsis ? 'is-ellipsis' : ''} ${customValueClass} ${showValue ? '' : 'ht-calendar__value--placeholder'}`"
            >
              {{ showValue || placeholder || translate('placeholder') }}
            </view>
            <ht-icon v-if="!disabled && !readonly" custom-class="ht-calendar__arrow" name="arrow-right" />
          </view>
          <view v-if="errorMessage" class="ht-calendar__error-message">{{ errorMessage }}</view>
        </view>
      </view>
    </view>
    <ht-action-sheet
      ...
      @close="close"
    >
      <view class="ht-calendar__header">
        <view v-if="!showTypeSwitch && shortcuts.length === 0" class="ht-calendar__title">{{ title || translate('title') }}</view>
        <view v-if="showTypeSwitch" class="ht-calendar__tabs">
          <ht-tabs ref="calendarTabs" v-model="currentTab" @change="handleTypeChange">
            <ht-tab :title="translate('day')" :name="translate('day')" />
            <ht-tab :title="translate('week')" :name="translate('week')" />
            <ht-tab :title="translate('month')" :name="translate('month')" />
          </ht-tabs>
        </view>
        <view v-if="shortcuts.length > 0" class="ht-calendar__shortcuts">
          <ht-tag
            v-for="(item, index) in shortcuts"
            :key="index"
            custom-class="ht-calendar__tag"
            type="primary"
            plain
            round
        ...
        <ht-icon custom-class="ht-calendar__close" name="add" @click="close" />
      </view>
      <view
        v-if="inited"
        :class="`ht-calendar__view  ${currentType.indexOf('range') > -1 ? 'is-range' : ''} ${showConfirm ? 'is-show-confirm' : ''}`"
      >
        <view v-if="range(type)" :class="`ht-calendar__range-label ${type === 'monthrange' ? 'is-monthrange' : ''}`">
          <view
            :class="`ht-calendar__range-label-item ${!calendarValue || !isArray(calendarValue) || !calendarValue[0] ? 'is-placeholder' : ''}`"
            style="text-align: right"
          >
            {{ rangeLabel[0] }}
          </view>
          <view class="ht-calendar__range-sperator">/</view>
          <view :class="`ht-calendar__range-label-item ${!calendarValue || !isArray(calendarValue) || !calendarValue[1] ? 'is-placeholder' : ''}`">
            {{ rangeLabel[1] }}
          </view>
        </view>
        <ht-calendar-view
      ...
      <view v-if="showConfirm" class="ht-calendar__confirm">
        <ht-button block :disabled="confirmBtnDisabled" @click="handleConfirm">{{ confirmText || translate('confirm') }}</ht-button>
      </view>
    </ht-action-sheet>
  </view>
</template>
...
import wdCalendarView from '../ht-calendar-view/ht-calendar-view.vue'
...
import { getWeekNumber, isRange } from '../ht-calendar-view/utils'
...
import type { CalendarType } from '../ht-calendar-view/types'
