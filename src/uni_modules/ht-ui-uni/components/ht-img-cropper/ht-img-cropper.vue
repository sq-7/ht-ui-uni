<template>
  <!-- 绘制的图片canvas -->
  <view v-if="modelValue" :class="`ht-img-cropper ${customClass}`" :style="customStyle" @touchmove="preventTouchMove">
    <!-- 展示在用户面前的裁剪框 -->
    <view class="ht-img-cropper__wrapper">
      <!-- 画出裁剪框 -->
      <view class="ht-img-cropper__cut">
        <!-- 上方阴影块 -->
        <view :class="`ht-img-cropper__cut--top ${IS_TOUCH_END ? '' : 'is-hightlight'}`" :style="`height: ${cutTop}px;`"></view>
        <view class="ht-img-cropper__cut--middle" :style="`height: ${cutHeight}px;`">
          <!-- 左侧阴影块 -->
          <view
            :class="`ht-img-cropper__cut--left ${IS_TOUCH_END ? '' : 'is-hightlight'}`"
            :style="`width: ${cutLeft}px; height: ${cutHeight}px;`"
          ></view>
          <!-- 裁剪框 -->
          <view class="ht-img-cropper__cut--body" :style="`width: ${cutWidth}px; height: ${cutHeight}px;`">
            <!-- 内部网格线 -->
          ...
          <!-- 右侧阴影块 -->
          <view :class="`ht-img-cropper__cut--right ${IS_TOUCH_END ? '' : 'is-hightlight'}`"></view>
        </view>

        <!-- 底部阴影块 -->
        <view :class="`ht-img-cropper__cut--bottom ${IS_TOUCH_END ? '' : 'is-hightlight'}`"></view>
      </view>
      <!-- 展示的传过来的图片: 控制图片的旋转角度(rotate)、缩放程度(imgScale)、移动位置(translate) -->
      <image
        :prop="isAnimation"
        :change:prop="animation ? animation.setAnimation : ''"
        class="ht-img-cropper__img"
        :src="imgSrc"
        :style="imageStyle"
        :lazy-load="false"
    ...
    <canvas
      :canvas-id="canvasId"
      :id="canvasId"
      class="ht-img-cropper__canvas"
      :disable-scroll="true"
      :style="`width: ${Number(canvasWidth) * canvasScale}px; height: ${Number(canvasHeight) * canvasScale}px;`"
    />
    <!-- 下方按钮 -->
    <view class="ht-img-cropper__footer">
      <ht-icon custom-class="ht-img-cropper__rotate" v-if="!disabledRotate" name="rotate" @click="handleRotate"></ht-icon>
      <view class="ht-img-cropper__footer--button">
        <view class="is-cancel" @click="handleCancel">{{ cancelButtonText || translate('cancel') }}</view>
        <ht-button size="small" :custom-style="buttonStyle" @click="handleConfirm">{{ confirmButtonText || translate('confirm') }}</ht-button>
      </view>
    </view>
  </view>
</template>
