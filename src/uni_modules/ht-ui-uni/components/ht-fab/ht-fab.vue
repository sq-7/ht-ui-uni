<template>
  <view
    ...
    :class="`ht-fab ${customClass}`"
    :style="rootStyle"
    @click.stop=""
  >
    <view @click.stop="" :style="{ visibility: inited ? 'visible' : 'hidden' }" id="trigger">
      <slot name="trigger" v-if="$slots.trigger"></slot>
      <ht-button v-else @click="handleClick" custom-class="ht-fab__trigger" round :type="type" :disabled="disabled">
        <ht-icon custom-class="ht-fab__icon" :name="isActive ? activeIcon : inactiveIcon"></ht-icon>
      </ht-button>
    </view>
    <ht-transition
      v-if="expandable"
      :enter-class="`ht-fab__transition-enter--${fabDirection}`"
      enter-active-class="ht-fab__transition-enter-active"
      :leave-to-class="`ht-fab__transition-leave-to--${fabDirection}`"
      leave-active-class="ht-fab__transition-leave-active"
      :custom-class="`ht-fab__actions ht-fab__actions--${fabDirection}`"
      :show="isActive"
      :duration="300"
    >
      <slot></slot>
