<template>
  <view :class="rootClass" :style="customStyle">
    <view class="ht-search__block">
      <slot name="prefix"></slot>
      <view class="ht-search__field">
        <view v-if="!placeholderLeft" :style="coverStyle" class="ht-search__cover" @click="closeCover">
          <ht-icon name="search" custom-class="ht-search__search-icon"></ht-icon>
          <text :class="`ht-search__placeholder-txt ${placeholderClass}`">{{ placeholder || translate('search') }}</text>
        </view>
        <ht-icon v-if="showInput || str || placeholderLeft" name="search" custom-class="ht-search__search-left-icon"></ht-icon>
        <input
          v-if="showInput || str || placeholderLeft"
          :placeholder="placeholder || translate('search')"
          :placeholder-class="`ht-search__placeholder-txt ${placeholderClass}`"
          :placeholder-style="placeholderStyle"
          confirm-type="search"
          v-model="str"
          :class="['ht-search__input', customInputClass]"
          @focus="searchFocus"
          @input="inputValue"
          @blur="searchBlur"
          @confirm="search"
          ...
          :focus="isFocused"
        />
        <ht-icon v-if="str" custom-class="ht-search__clear ht-search__clear-icon" name="error-fill" @click="clearSearch" />
      </view>
    </view>

    <slot v-if="!hideCancel" name="suffix">
      <view class="ht-search__cancel" @click="handleCancel">
...
const rootClass = computed(() => {
  return `ht-search  ${props.light ? 'is-light' : ''}  ${props.hideCancel ? 'is-without-cancel' : ''} ${props.customClass}`
})
