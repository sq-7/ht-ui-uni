<template>
  <view :class="`ht-tab ${customClass}`" :style="customStyle">
    <view :class="['ht-tab__body', { 'ht-tab__body--inactive': !active }]" v-if="shouldBeRender" :style="tabBodyStyle">
      <slot />
    </view>
  </view>
</template>
...
import { TABS_KEY } from '../ht-tabs/types'
...
watch(
  ...
  (newValue) => {
    if (isDef(newValue) && !isNumber(newValue) && !isString(newValue)) {
      console.error('[wot design] error(ht-tab): the type of name should be number or string')
      return
    }
    if (tabs) {
      checkName(proxy)
