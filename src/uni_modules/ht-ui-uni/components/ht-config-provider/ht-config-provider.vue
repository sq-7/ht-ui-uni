 ...
 * @Description:
 * @FilePath: \ht-ui-uni\src\uni_modules\ht-ui-uni\components\ht-config-provider\ht-config-provider.vue
 * 记得注释
-->
<template>
...
const themeClass = computed(() => {
  return `ht-theme-${props.theme} ${props.customClass}`
})
...
const mapThemeVarsToCSSVars = (themeVars: Record<string, string>) => {
  ...
  Object.keys(themeVars).forEach((key) => {
    cssVars[`--ht-${kebabCase(key)}`] = themeVars[key]
  })
