<template>
  <view :class="rootClass" :style="customStyle" :id="sliderId">
    ...
    <view :id="sliderId" style="flex: 1" :class="rootClass">
      <!-- #endif -->
      <view :class="`ht-slider__label-min ${customMinClass}`" v-if="!hideMinMax">
        {{ minValue }}
      </view>
      <view class="ht-slider__bar-wrapper" :style="barWrapperStyle">
        <view class="ht-slider__bar" :style="barCustomStyle"></view>
        <!-- 左边 -->
        <view
          class="ht-slider__button-wrapper"
        ...
        >
          <view class="ht-slider__label" v-if="!hideLabel">{{ leftNewValue }}</view>
          <view class="ht-slider__button" />
        </view>
        <!-- 右边 -->
        <view
          v-if="showRight"
        ...
        >
          <view class="ht-slider__label" v-if="!hideLabel">{{ rightNewValue }}</view>
          <view class="ht-slider__button" />
        </view>
      </view>
      <view :class="`ht-slider__label-max ${customMaxClass}`" v-if="!hideMinMax">
        {{ maxValue }}
      </view>
      <!-- #ifdef MP-DINGTALK -->
    </view>
...
watch(
  ...
  (newValue) => {
    if (newValue <= props.min) {
      ...
      console.warn('[wot design] warning(ht-slider): max value must be greater than min value')
    } else {
...
watch(
  ...
  (newValue) => {
    if (newValue >= props.max) {
      ...
      console.warn('[wot design] warning(ht-slider): min value must be less than max value')
    } else {
...
watch(
  ...
  (newValue) => {
    if (newValue < 1) {
      stepValue.value = 1
      console.warn('[wot design] warning(ht-slider): step must be greater than 0')
    } else {
...
watch(
  ...
  (newValue) => {
    ...
    if (newValue === null || newValue === undefined) {
      ...
      console.warn('[wot design] warning(ht-slider): value can nott be null or undefined')
    } else if (isArray(newValue) && newValue.length !== 2) {
      console.warn('[wot design] warning(ht-slider): value must be dyadic array')
    } else if (!isNumber(newValue) && !isArray(newValue)) {
      emit('update:modelValue', currentValue.value)
      console.warn('[wot design] warning(ht-slider): value must be dyadic array Or Number')
    }
    // 动态传值后修改
    if (isArray(newValue)) {
...
const rootClass = computed(() => {
  const rootClass = `ht-slider ${!props.hideLabel ? 'ht-slider__has-label' : ''} ${props.disabled ? 'ht-slider--disabled' : ''} ${props.customClass}`
  return rootClass
})
