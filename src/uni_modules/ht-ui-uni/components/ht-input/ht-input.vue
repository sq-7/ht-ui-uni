<template>
  <view :class="rootClass" :style="customStyle" @click="handleClick">
    <view v-if="label || $slots.label" :class="labelClass" :style="labelStyle">
      <view v-if="prefixIcon || $slots.prefix" class="ht-input__prefix">
        <ht-icon v-if="prefixIcon && !$slots.prefix" custom-class="ht-input__icon" :name="prefixIcon" @click="onClickPrefixIcon" />
        <slot v-else name="prefix"></slot>
      </view>
      <view class="ht-input__label-inner">
        <template v-if="label && !$slots.label">{{ label }}</template>
        <slot v-else name="label"></slot>
      </view>
    </view>
    <view class="ht-input__body">
      <view class="ht-input__value">
        <view v-if="(prefixIcon || $slots.prefix) && !label" class="ht-input__prefix">
          <ht-icon v-if="prefixIcon && !$slots.prefix" custom-class="ht-input__icon" :name="prefixIcon" @click="onClickPrefixIcon" />
          <slot v-else name="prefix"></slot>
        </view>
        <input
          :class="[
            'ht-input__inner',
            prefixIcon ? 'ht-input__inner--prefix' : '',
            showWordCount ? 'ht-input__inner--count' : '',
        ...
        />
        <view v-if="props.readonly" class="ht-input__readonly-mask" />
        <view v-if="showClear || showPassword || suffixIcon || showWordCount || $slots.suffix" class="ht-input__suffix">
          <ht-icon v-if="showClear" custom-class="ht-input__clear" name="error-fill" @click="handleClear" />
          <ht-icon v-if="showPassword" custom-class="ht-input__icon" :name="isPwdVisible ? 'view' : 'eye-close'" @click="togglePwdVisible" />
          <view v-if="showWordCount" class="ht-input__count">
            <text
              :class="[
              inputValue && String(inputValue).length > 0 ? 'ht-input__count-current' : '',
          ...
          <ht-icon v-if="suffixIcon && !$slots.suffix" custom-class="ht-input__icon" :name="suffixIcon" @click="onClickSuffixIcon" />
          <slot v-else name="suffix"></slot>
        </view>
      </view>
      <view v-if="errorMessage" class="ht-input__error-message">{{ errorMessage }}</view>
    </view>
  </view>
</template>
...
const rootClass = computed(() => {
  return `ht-input  ${props.label || slots.label ? 'is-cell' : ''} ${props.center ? 'is-center' : ''} ${cell.border.value ? 'is-border' : ''} ${
...
const labelClass = computed(() => {
  return `ht-input__label ${props.customLabelClass} ${isRequired.value ? 'is-required' : ''}`
})

const inputPlaceholderClass = computed(() => {
  return `ht-input__placeholder  ${props.placeholderClass}`
})
