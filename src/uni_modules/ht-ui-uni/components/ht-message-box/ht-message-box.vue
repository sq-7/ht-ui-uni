    ...
    >
      <view :class="rootClass">
        <view :class="bodyClass">
          <view v-if="messageState.title" class="ht-message-box__title">
            {{ messageState.title }}
          </view>
          <view class="ht-message-box__content">
            <block v-if="messageState.type === 'prompt'">
              ...
              />
              <view v-if="messageState.showErr" class="ht-message-box__input-error">
        ...
        <view :class="`ht-message-box__actions ${messageState.showCancelButton ? 'ht-message-box__flex' : 'ht-message-box__block'}`">
          <ht-button v-bind="customCancelProps" v-if="messageState.showCancelButton" @click="toggleModal('cancel')">
            {{ messageState.cancelButtonText || translate('cancel') }}
          </ht-button>
          <ht-button v-bind="customConfirmProps" @click="toggleModal('confirm')">
...
import wdInput from '../ht-input/ht-input.vue'
...
import type { ButtonProps } from '../ht-button/types'
...
const rootClass = computed(() => {
  return `ht-message-box__container ${props.customClass}`
})

const bodyClass = computed(() => {
  return `ht-message-box__body ${!messageState.title ? 'is-no-title' : ''} ${messageState.type === 'prompt' ? 'is-prompt' : ''}`
})
...
const customConfirmProps = computed(() => {
  ...
  buttonProps.customClass = `${buttonProps.customClass || ''} ht-message-box__actions-btn`
  return buttonProps
})
