<template>
  <view :class="`ht-picker-view ${customClass}`" :style="customStyle">
    <view class="ht-picker-view__loading" v-if="loading">
      <ht-loading :color="loadingColor" />
    </view>
    <view :style="`height: ${columnsHeight - 20}px;`">
      <picker-view
        mask-class="ht-picker-view__mask"
        indicator-class="ht-picker-view__roller"
        ...
        @pickend="onPickEnd"
      >
        <picker-view-column v-for="(col, colIndex) in formatColumns" :key="colIndex" class="ht-picker-view-column">
          <view
            v-for="(row, rowIndex) in col"
            :key="rowIndex"
            :class="`ht-picker-view-column__item ${row['disabled'] ? 'ht-picker-view-column__item--disabled' : ''}  ${
              selectedIndex[colIndex] == rowIndex ? 'ht-picker-view-column__item--active' : ''
            }`"
