<template>
  <view
    :class="`ht-picker ${disabled ? 'is-disabled' : ''} ${size ? 'is-' + size : ''}  ${cell.border.value ? 'is-border' : ''} ${
      alignRight ? 'is-align-right' : ''
    } ${error ? 'is-error' : ''} ${customClass}`"
    :style="customStyle"
  >
    <!--文案-->
    <view class="ht-picker__field" @click="showPopup">
      <slot v-if="$slots.default"></slot>
      <view v-else :class="['ht-picker__cell', customCellClass]">
        <view
          v-if="label || $slots.label"
          :class="`ht-picker__label ${customLabelClass} ${isRequired ? 'is-required' : ''}`"
          :style="labelWidth ? 'min-width:' + labelWidth + ';max-width:' + labelWidth + ';' : ''"
        >
          <slot v-if="$slots.label" name="label"></slot>
          <block v-else>{{ label }}</block>
        </view>
        <view class="ht-picker__body">
          <view class="ht-picker__value-wraper">
            <view :class="`ht-picker__value ${customValueClass}`">
              <template v-if="region">
                <view v-if="isArray(showValue)">
                  <text :class="showValue[0] ? '' : 'ht-picker__placeholder'">
                    {{ showValue[0] ? showValue[0] : placeholder || translate('placeholder') }}
                  </text>
                  {{ translate('to') }}
                  <text :class="showValue[1] ? '' : 'ht-picker__placeholder'">
                    {{ showValue[1] ? showValue[1] : placeholder || translate('placeholder') }}
                  </text>
                </view>
                <view v-else class="ht-picker__placeholder">
                  {{ placeholder || translate('placeholder') }}
                </view>
              </template>
              <view v-else :class="showValue ? '' : 'ht-picker__placeholder'">
                {{ showValue ? showValue : placeholder || translate('placeholder') }}
              </view>
            </view>
            <ht-icon v-if="!disabled && !readonly" custom-class="ht-picker__arrow" name="arrow-right" />
          </view>
          <view v-if="errorMessage" class="ht-picker__error-message">{{ errorMessage }}</view>
        </view>
      </view>
    </view>
    ...
    <ht-popup
      ...
      :z-index="zIndex"
      @close="onCancel"
      custom-class="ht-picker__popup"
    >
      <view class="ht-picker__wraper">
        <!--toolBar-->
        <view class="ht-picker__toolbar" @touchmove="noop">
          <!--取消按钮-->
          <view class="ht-picker__action ht-picker__action--cancel" @click="onCancel">
            {{ cancelButtonText || translate('cancel') }}
          </view>
          <!--标题-->
          <view v-if="title" class="ht-picker__title">{{ title }}</view>
          <!--确定按钮-->
          <view :class="`ht-picker__action ${loading || isLoading ? 'is-loading' : ''}`" @click="onConfirm">
            {{ confirmButtonText || translate('confirm') }}
          </view>
        </view>
        <!-- 区域选择tab展示 -->
        <view v-if="region" class="ht-picker__region-tabs">
          <view :class="`ht-picker__region ${showStart ? 'is-active' : ''} `" @click="tabChange">
            <view>{{ translate('start') }}</view>
            <view class="ht-picker__region-time">{{ showTabLabel[0] }}</view>
          </view>
          <view :class="`ht-picker__region ${showStart ? '' : 'is-active'}`" @click="tabChange">
            <view>{{ translate('end') }}</view>
            <view class="ht-picker__region-time">{{ showTabLabel[1] }}</view>
          </view>
        </view>
        <!--datetimePickerView-->
        <view :class="showStart ? 'ht-picker__show' : 'ht-picker__hidden'">
          <ht-datetime-picker-view
        ...
        <view :class="showStart ? 'ht-picker__hidden' : 'ht-picker__show'">
          <ht-datetime-picker-view
            :custom-class="customViewClass"
...
import wdDatetimePickerView from '../ht-datetime-picker-view/ht-datetime-picker-view.vue'
...
} from '../ht-datetime-picker-view/types'
