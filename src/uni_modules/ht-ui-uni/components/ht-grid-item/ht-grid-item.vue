<template>
  <view :class="`ht-grid-item ${border && !gutter ? itemClass : ''} ${customClass}`" @click="click" :style="`${style};${customStyle}`">
    <view
      :class="`ht-grid-item__content ${square ? 'is-square' : ''} ${border && gutter > 0 ? 'is-round' : ''}`"
      :style="gutterContentStyle"
      :hover-class="hoverClass"
    >
      <slot v-if="useSlot" />
      <block v-else>
        <view :style="'width:' + iconSize + '; height: ' + iconSize" class="ht-grid-item__wrapper">
          <ht-badge custom-class="badge" v-bind="customBadgeProps">
            <template v-if="useIconSlot">
              <slot name="icon" />
            </template>
            <ht-icon v-else :name="icon" :size="iconSize" :custom-class="customIcon" />
          </ht-badge>
        </view>
        <slot name="text" v-if="useTextSlot" />
        <view v-else class="ht-grid-item__text custom-text">{{ text }}</view>
      </block>
    </view>
  </view>
</template>
...
import { GRID_KEY } from '../ht-grid/types'
  ...
  if (url) {
    switch (linkType) {
      ...
      case 'switchTab':
        ...
        break
      default:
        console.error(`[ht-design] warning(ht-grid-item): linkType can not be ${linkType}`)
        break
    }
  }
}
...
const hoverClass = computed(() => {
  if (grid?.props.clickable) {
    return grid.props.hoverClass ? grid.props.hoverClass : 'ht-grid-item__content--hover'
  }
  return 'none'
})
