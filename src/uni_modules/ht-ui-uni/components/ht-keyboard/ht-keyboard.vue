<template>
  ...
  >
    <view :class="`ht-keyboard ${customClass}`" :style="customStyle">
      <view class="ht-keyboard__header" v-if="showTitle">
        <slot name="title">
          <text class="ht-keyboard__title">{{ title }}</text>
        </slot>
        <view class="ht-keyboard__close" hover-class="ht-keyboard__close--hover" v-if="showClose" @click="handleClose">
          <text>{{ closeText }}</text>
        </view>
      </view>
      <template v-if="mode !== 'car'">
        <view class="ht-keyboard__body">
          <view class="ht-keyboard__keys">
            <ht-key v-for="key in keys" :key="key.text" :text="key.text" :type="key.type" :wider="key.wider" @press="handlePress"></ht-key>
          </view>
          <view class="ht-keyboard__sidebar" v-if="mode === 'custom'">
      ...
      <template v-if="mode === 'car'">
        <view class="ht-keyboard-car__body">
          <view class="ht-keyboard-car__keys">
            <ht-key v-for="key in keys" :key="key.text" :text="key.text" :type="key.type" :wider="key.wider" @press="handlePress"></ht-key>
          </view>
        </view>
      </template>
