<template>
  <view class="ht-signature">
    <view class="ht-signature__content">
      <!-- #ifdef MP-WEIXIN -->
      <canvas
        class="ht-signature__content-canvas"
        :style="canvasStyle"
    ...
    <view class="ht-signature__footer">
      <slot
        name="footer"
        :clear="clear"
      ...
      >
        <block v-if="enableHistory">
          <ht-button size="small" plain @click="revoke" :disabled="lines.length <= 0">
            {{ revokeText || translate('revokeText') }}
          </ht-button>
          <ht-button size="small" plain @click="restore" :disabled="redoLines.length <= 0">
            {{ restoreText || translate('restoreText') }}
          </ht-button>
        </block>
        <ht-button size="small" plain @click="clear">{{ clearText || translate('clearText') }}</ht-button>
        <ht-button size="small" @click="confirmSignature">{{ confirmText || translate('confirmText') }}</ht-button>
      </slot>
    </view>
  </view>
</template>
