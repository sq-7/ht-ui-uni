<template>
  <view
    v-if="showWrapper"
    :class="`ht-drop-item  ${customClass}`"
    ...
    >
      <view v-if="options.length">
        <view
          ...
          :key="index"
          @click="choose(index)"
          :class="`ht-drop-item__option ${(item[valueKey] !== '' ? item[valueKey] : item) === modelValue ? 'is-active' : ''}`"
        >
          <view :class="`ht-drop-item__title ${customTitle}`">
            <text>{{ item[labelKey] ? item[labelKey] : item }}</text>
            <text v-if="item[tipKey]" class="ht-drop-item__tip">{{ item[tipKey] }}</text>
          </view>
          <ht-icon
            v-if="(item[valueKey] !== '' ? item[valueKey] : item) === modelValue"
            :name="iconName"
            size="20px"
            :class="`ht-drop-item__icon ${customIcon}`"
          />
        </view>
      </view>
      <slot v-else />
...
import type { PopupType } from '../ht-popup/types'
import { useParent } from '../composables/useParent'
import { DROP_MENU_KEY } from '../ht-drop-menu/types'
...
watch(
  ...
  (newValue) => {
    if (isDef(newValue) && typeof newValue !== 'number' && typeof newValue !== 'string') {
      console.error('[ht-design] warning(ht-drop-menu-item): the type of value should be a number or a string.')
    }
  },
  {
    deep: true,
