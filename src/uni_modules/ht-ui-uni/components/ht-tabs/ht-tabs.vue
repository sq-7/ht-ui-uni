<template>
  <template v-if="sticky">
    <ht-sticky-box>
      <view
        :class="`ht-tabs ${customClass} ${innerSlidable ? 'is-slide' : ''} ${mapNum < children.length && mapNum !== 0 ? 'is-map' : ''}`"
        :style="customStyle"
      >
        <ht-sticky :offset-top="offsetTop">
          <view class="ht-tabs__nav ht-tabs__nav--sticky">
            <view class="ht-tabs__nav--wrap">
              <scroll-view :scroll-x="innerSlidable" scroll-with-animation :scroll-left="state.scrollLeft">
                <view class="ht-tabs__nav-container">
                  <view
                    @click="handleSelect(index)"
                    v-for="(item, index) in children"
                    :key="index"
                    :class="`ht-tabs__nav-item  ${state.activeIndex === index ? 'is-active' : ''} ${item.disabled ? 'is-disabled' : ''}`"
                    :style="state.activeIndex === index ? (color ? 'color:' + color : '') : inactiveColor ? 'color:' + inactiveColor : ''"
                  >
                    <ht-badge v-if="item.badgeProps" v-bind="item.badgeProps">
                      <text class="ht-tabs__nav-item-text">{{ item.title }}</text>
                    </ht-badge>
                    <text v-else class="ht-tabs__nav-item-text">{{ item.title }}</text>

                    <view class="ht-tabs__line ht-tabs__line--inner" v-if="state.activeIndex === index && state.useInnerLine"></view>
                  </view>
                  <view class="ht-tabs__line" :style="state.lineStyle"></view>
                </view>
              </scroll-view>
            </view>
            <view class="ht-tabs__map" v-if="mapNum < children.length && mapNum !== 0">
              <view :class="`ht-tabs__map-btn  ${state.animating ? 'is-open' : ''}`" @click="toggleMap">
                <view :class="`ht-tabs__map-arrow  ${state.animating ? 'is-open' : ''}`">
                  <ht-icon name="arrow-down" />
                </view>
              </view>
              <view class="ht-tabs__map-header" :style="`${state.mapShow ? '' : 'display:none;'}  ${state.animating ? 'opacity:1;' : ''}`">
                {{ mapTitle || translate('all') }}
              </view>
              <view :class="`ht-tabs__map-body  ${state.animating ? 'is-open' : ''}`" :style="state.mapShow ? '' : 'display:none'">
                <view class="ht-tabs__map-nav-item" v-for="(item, index) in children" :key="index" @click="handleSelect(index)">
                  <view
                    :class="`ht-tabs__map-nav-btn ${state.activeIndex === index ? 'is-active' : ''}  ${item.disabled ? 'is-disabled' : ''}`"
                    :style="
        ...
        <view class="ht-tabs__container" @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd" @touchcancel="onTouchEnd">
          <view :class="['ht-tabs__body', animated ? 'is-animated' : '']" :style="bodyStyle">
            <slot />
          </view>
        </view>
  ...
  <template v-else>
    <view :class="`ht-tabs ${customClass} ${innerSlidable ? 'is-slide' : ''} ${mapNum < children.length && mapNum !== 0 ? 'is-map' : ''}`">
      <view class="ht-tabs__nav">
        <view class="ht-tabs__nav--wrap">
          <scroll-view :scroll-x="innerSlidable" scroll-with-animation :scroll-left="state.scrollLeft">
            <view class="ht-tabs__nav-container">
              <view
                ...
                :style="state.activeIndex === index ? (color ? 'color:' + color : '') : inactiveColor ? 'color:' + inactiveColor : ''"
              >
                <ht-badge custom-class="ht-tabs__nav-item-badge" v-if="item.badgeProps" v-bind="item.badgeProps">
                  <text class="ht-tabs__nav-item-text">{{ item.title }}</text>
                </ht-badge>
                <text v-else class="ht-tabs__nav-item-text">{{ item.title }}</text>
                <view class="ht-tabs__line ht-tabs__line--inner" v-if="state.activeIndex === index && state.useInnerLine"></view>
        ...
        <view class="ht-tabs__map" v-if="mapNum < children.length && mapNum !== 0">
          <view class="ht-tabs__map-btn" @click="toggleMap">
            <view :class="`ht-tabs__map-arrow ${state.animating ? 'is-open' : ''}`">
              <ht-icon name="arrow-down" />
            </view>
          </view>
          ...
          <view :class="`ht-tabs__map-body ${state.animating ? 'is-open' : ''}`" :style="state.mapShow ? '' : 'display:none'">
            <view class="ht-tabs__map-nav-item" v-for="(item, index) in children" :key="index" @click="handleSelect(index)">
              <view :class="`ht-tabs__map-nav-btn ${state.activeIndex === index ? 'is-active' : ''}  ${item.disabled ? 'is-disabled' : ''}`">
                {{ item.title }}
              </view>
            </view>
          </view>
      ...
      <view class="ht-tabs__mask" :style="`${state.mapShow ? '' : 'display:none;'}  ${state.animating ? 'opacity:1' : ''}`" @click="toggleMap"></view>
    </view>
  </template>
</template>
<script lang="ts">
...
import wdSticky from '../ht-sticky/ht-sticky.vue'
import wdStickyBox from '../ht-sticky-box/ht-sticky-box.vue'
...
const $item = '.ht-tabs__nav-item'
const $itemText = '.ht-tabs__nav-item-text'
const $container = '.ht-tabs__nav-container'
...
watch(
  ...
  (newValue) => {
    if (!isNumber(newValue) && !isString(newValue)) {
      console.error('[wot design] error(ht-tabs): the type of value should be number or string')
    }
    // 保证不为非空字符串，小于0的数字
    if (newValue === '' || !isDef(newValue)) {
      // eslint-disable-next-line quotes
      console.error("[wot design] error(ht-tabs): tabs's value cannot be '' null or undefined")
    }
    if (typeof newValue === 'number' && newValue < 0) {
      // eslint-disable-next-line quotes
      console.error("[wot design] error(ht-tabs): tabs's value cannot be less than zero")
    }
  },
  {
    immediate: true,
...
async function updateLineStyle(animation: boolean = true) {
  ...
  try {
    ...
    console.error('[wot design] error(ht-tabs): update line style failed', error)
  }
}
...
function getActiveIndex(value: number | string) {
  ...
  if (isNumber(value) && value >= children.length) {
    ...
    console.error('[wot design] warning(ht-tabs): the type of tabs\' value is Number shouldn\'t be less than its children')
    value = 0
  }
  // 如果是字符串直接匹配，匹配不到用0兜底
  if (isString(value)) {
