<template>
  <view :class="`ht-collapse-item ${disabled ? 'is-disabled' : ''} is-border ${customClass}`" :style="customStyle">
    <view
      :class="`ht-collapse-item__header ${expanded ? 'is-expanded' : ''} ${isFirst ? 'ht-collapse-item__header-first' : ''} ${
        $slots.title ? 'is-custom' : ''
      }`"
      @click="handleClick"
    >
      <slot name="title" :expanded="expanded" :disabled="disabled" :isFirst="isFirst">
        <text class="ht-collapse-item__title">{{ title }}</text>
        <ht-icon name="arrow-down" :custom-class="`ht-collapse-item__arrow ${expanded ? 'is-retract' : ''}`" />
      </slot>
    </view>
    <view class="ht-collapse-item__wrapper" :style="contentStyle" @transitionend="handleTransitionEnd">
      <view class="ht-collapse-item__body" :class="customBodyClass" :style="customBodyStyle" :id="collapseId">
        <slot />
      </view>
    </view>
  </view>
...
import { COLLAPSE_KEY } from '../ht-collapse/types'
