<template>
  <view :class="rootClass" :style="customStyle">
    <view v-if="label || $slots.label" :class="labelClass" :style="labelStyle">
      <view v-if="prefixIcon || $slots.prefix" class="ht-textarea__prefix">
        <ht-icon v-if="prefixIcon && !$slots.prefix" custom-class="ht-textarea__icon" :name="prefixIcon" @click="onClickPrefixIcon" />
        <slot v-else name="prefix"></slot>
      </view>
      <view class="ht-textarea__label-inner">
    ...
    <!-- 文本域 -->
    <view :class="`ht-textarea__value ${showClear ? 'is-suffix' : ''} ${customTextareaContainerClass} ${showWordCount ? 'is-show-limit' : ''}`">
      <textarea
        :class="`ht-textarea__inner ${customTextareaClass}`"
        v-model="inputValue"
        :show-count="false"
      ...
      />
      <view v-if="errorMessage" class="ht-textarea__error-message">{{ errorMessage }}</view>

      <view v-if="props.readonly" class="ht-textarea__readonly-mask" />
      <view class="ht-textarea__suffix">
        <ht-icon v-if="showClear" custom-class="ht-textarea__clear" name="error-fill" @click="handleClear" />
        <view v-if="showWordCount" class="ht-textarea__count">
...
const currentLength = computed(() => {
   ...
   * @link https://github.com/Moonofweisheng/ht-ui-uni/issues/933
   */
  return Array.from(String(formatValue(props.modelValue) || '')).length
})

const rootClass = computed(() => {
  return `ht-textarea   ${props.label || slots.label ? 'is-cell' : ''} ${props.center ? 'is-center' : ''} ${cell.border.value ? 'is-border' : ''} ${
...
const labelClass = computed(() => {
  return `ht-textarea__label ${props.customLabelClass} ${isRequired.value ? 'is-required' : ''}`
})

const inputPlaceholderClass = computed(() => {
  return `ht-textarea__placeholder  ${props.placeholderClass}`
})

const countClass = computed(() => {
  return `${currentLength.value > 0 ? 'ht-textarea__count-current' : ''} ${currentLength.value > props.maxlength ? 'is-error' : ''}`
})
