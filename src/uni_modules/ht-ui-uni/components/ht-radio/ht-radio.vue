<template>
  <view
    :class="`ht-radio ${cellValue ? 'is-cell-radio' : ''} ${cellValue && shapeValue == 'button' ? 'is-button-radio' : ''} ${
  ...
  >
    ...
    <view class="ht-radio__shape" :style="isChecked && !disabledValue ? 'color: ' + checkedColorValue : ''">
      <ht-icon v-if="shapeValue === 'check'" :style="isChecked && !disabledValue ? 'color: ' + checkedColorValue : ''" name="check"></ht-icon>
    </view>
  </view>
</template>
<script lang="ts">
...
import { RADIO_GROUP_KEY } from '../ht-radio-group/types'
