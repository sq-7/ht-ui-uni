 ...
 * @Description:
 * @FilePath: /ht-ui-uni/src/uni_modules/ht-ui-uni/components/ht-skeleton/ht-skeleton.vue
 * 记得注释
-->
<template>
  <view :class="`ht-skeleton ${customClass}`" :style="customStyle">
    <view class="ht-skeleton__content" v-if="show">
      <view class="ht-skeleton__row" v-for="(row, index) of parsedRowCols" :key="`row-${index}`">
...
function getColItemClass(rowCol: SkeletonRowColObj) {
  return ['ht-skeleton__col', `ht-skeleton--type-${rowCol.type || 'text'}`, { [`ht-skeleton--animation-${props.animation}`]: props.animation }]
}
