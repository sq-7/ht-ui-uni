<template>
  <view class="ht-index-bar" :id="indexBarId">
    <!-- #ifdef MP-DINGTALK -->
    <view class="ht-index-bar" :id="indexBarId">
      <!-- #endif -->
      <scroll-view :scrollTop="scrollState.scrollTop" :scroll-y="true" class="ht-index-bar__content" @scroll="hanleScroll">
        <slot></slot>
      </scroll-view>
      <view
        class="ht-index-bar__sidebar"
        @touchstart.stop.prevent="handleTouchStart"
        @touchmove.stop.prevent="handleTouchMove"
        @touchend.stop.prevent="handleTouchEnd"
        @touchcancel.stop.prevent="handleTouchEnd"
      >
        <view class="ht-index-bar__index" :class="{ 'is-active': item.index === state.activeIndex }" v-for="item in children" :key="item.index">
          {{ item.index }}
        </view>
      </view>
      <!-- #ifdef MP-DINGTALK -->
...
function init() {
  setTimeout(() => {
    Promise.all([
      ...
      getRect('.ht-index-bar__sidebar', false, proxy),
      getRect('.ht-index-bar__index', false, proxy)
