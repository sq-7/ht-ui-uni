<template>
  <view :class="{ 'ht-tabbar__placeholder': fixed && placeholder && safeAreaInsetBottom && shape === 'round' }" :style="{ height: addUnit(height) }">
    <view
      :class="`ht-tabbar ht-tabbar--${shape} ${customClass} ${fixed ? 'is-fixed' : ''} ${safeAreaInsetBottom ? 'is-safe' : ''} ${
        bordered ? 'is-border' : ''
      }`"
      :style="rootStyle"
    >
...
import type { TabbarItem } from '../ht-tabbar-item/types'
...
function setPlaceholderHeight() {
  ...
  getRect('.ht-tabbar', false, proxy).then((res) => {
