<template>
  <view :class="`ht-input-number ${customClass} ${disabled ? 'is-disabled' : ''} ${withoutInput ? 'is-without-input' : ''}`" :style="customStyle">
    <!-- 减号按钮 -->
    <view
      :class="`ht-input-number__action ${minDisabled || disableMinus ? 'is-disabled' : ''}`"
      @click="handleClick('sub')"
      @touchstart="handleTouchStart('sub')"
      @touchend.stop="handleTouchEnd"
    >
      <ht-icon name="decrease" custom-class="ht-input-number__action-icon"></ht-icon>
    </view>
    <!-- 输入框 -->
    <view v-if="!withoutInput" class="ht-input-number__inner" @click.stop="">
      <input
        class="ht-input-number__input"
        ...
        @blur="handleBlur"
      />
      <view class="ht-input-number__input-border"></view>
    </view>
    <!-- 加号按钮 -->
    <view
      :class="`ht-input-number__action ${maxDisabled || disablePlus ? 'is-disabled' : ''}`"
      @click="handleClick('add')"
      @touchstart="handleTouchStart('add')"
      @touchend.stop="handleTouchEnd"
    >
      <ht-icon name="add" custom-class="ht-input-number__action-icon"></ht-icon>
    </view>
  </view>
</template>
