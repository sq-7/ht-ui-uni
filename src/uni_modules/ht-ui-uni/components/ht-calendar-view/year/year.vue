<template>
  <ht-toast selector="ht-year" />

  <view class="ht-year year">
    <view class="ht-year__title" v-if="showTitle">{{ yearTitle(date) }}</view>
    <view class="ht-year__months">
      <view
        v-for="(item, index) in months"
        :key="index"
        :class="`ht-year__month ${item.disabled ? 'is-disabled' : ''} ${item.isLastRow ? 'is-last-row' : ''} ${
          item.type ? monthTypeClass(item.type) : ''
        }`"
        @click="handleDateClick(index)"
      >
        <view class="ht-year__month-top">{{ item.topInfo }}</view>
        <view class="ht-year__month-text">{{ getMonthLabel(item.date) }}</view>
        <view class="ht-year__month-bottom">{{ item.bottomInfo }}</view>
      </view>
    </view>
  </view>
</template>
...
import wdToast from '../../ht-toast/ht-toast.vue'
...
function setMonths() {
  ...
  if (props.type.indexOf('range') > -1 && value && !isArray(value)) {
    console.error('[ht-design] value should be array when type is range')
    return
  }
...
function getFormatterDate(date: number, month: number, type?: CalendarDayType) {
  ...
  if (props.formatter) {
    if (isFunction(props.formatter)) {
      ...
      console.error('[ht-design] error(ht-calendar-view): the formatter prop of ht-calendar-view should be a function')
    }
  }
