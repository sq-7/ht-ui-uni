<template>
  <view>
    <ht-toast selector="ht-month" />
    <view class="month">
      <view class="ht-month">
        <view class="ht-month__title" v-if="showTitle">{{ monthTitle(date) }}</view>
        <view class="ht-month__days">
          <view
            v-for="(item, index) in days"
            :key="index"
            :class="`ht-month__day ${item.disabled ? 'is-disabled' : ''} ${item.isLastRow ? 'is-last-row' : ''} ${
              item.type ? dayTypeClass(item.type) : ''
            }`"
            :style="index === 0 ? firstDayStyle : ''"
            @click="handleDateClick(index)"
          >
            <view class="ht-month__day-container">
              <view class="ht-month__day-top">{{ item.topInfo }}</view>
              <view class="ht-month__day-text">
                {{ item.text }}
              </view>
              <view class="ht-month__day-bottom">{{ item.bottomInfo }}</view>
            </view>
          </view>
        </view>
      </view>
