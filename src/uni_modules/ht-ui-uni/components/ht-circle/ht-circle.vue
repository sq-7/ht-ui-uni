<template>
  <view :class="`ht-circle ${customClass}`" :style="customStyle">
    <!-- #ifdef MP-WEIXIN -->
    ...
    <view v-if="!text" class="ht-circle__text">
      <!-- 自定义提示内容 -->
      <slot></slot>
    </view>

    <text v-else class="ht-circle__text">
      {{ text }}
    </text>
  </view>
</template>
...
const canvasId = ref<string>(`ht-circle${uuid()}`) // canvasId
