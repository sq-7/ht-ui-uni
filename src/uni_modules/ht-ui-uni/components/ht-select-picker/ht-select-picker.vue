<template>
  <view :class="`ht-select-picker ${cell.border.value ? 'is-border' : ''} ${customClass}`" :style="customStyle">
    <view class="ht-select-picker__field" @click="open">
      <slot v-if="useDefaultSlot"></slot>
      <view
        v-else
        :class="`ht-select-picker__cell ${disabled && 'is-disabled'} ${props.readonly && 'is-readonly'} ${alignRight && 'is-align-right'} ${
          error && 'is-error'
        } ${size && 'is-' + size}`"
      >
        <view
          v-if="label || useLabelSlot"
          :class="`ht-select-picker__label ${isRequired && 'is-required'} ${customLabelClass}`"
          :style="labelWidth ? 'min-width:' + labelWidth + ';max-width:' + labelWidth + ';' : ''"
        >
          <block v-if="label">{{ label }}</block>
          <slot v-else name="label"></slot>
        </view>
        <view class="ht-select-picker__body">
          <view class="ht-select-picker__value-wraper">
            <view
              :class="`ht-select-picker__value ${ellipsis && 'is-ellipsis'} ${customValueClass} ${
                showValue ? '' : 'ht-select-picker__value--placeholder'
              }`"
            >
              {{ showValue || placeholder || translate('placeholder') }}
            </view>
            <ht-icon v-if="showArrow" custom-class="ht-select-picker__arrow" name="arrow-right" />
            <view v-else-if="showClear" @click.stop="handleClear">
              <ht-icon custom-class="ht-select-picker__clear" name="error-fill" />
            </view>
          </view>

          <view v-if="errorMessage" class="ht-select-picker__error-message">{{ errorMessage }}</view>
        </view>
      </view>
    </view>
    <ht-action-sheet
      ...
      custom-header-class="ht-select-picker__header"
    >
      <ht-search
        v-if="filterable"
        v-model="filterVal"
      ...
      />
      <scroll-view
        :class="`ht-select-picker__wrapper ${filterable ? 'is-filterable' : ''} ${loading ? 'is-loading' : ''} ${customContentClass}`"
        :scroll-y="!loading"
        :scroll-top="scrollTop"
        :scroll-with-animation="true"
      >
        <!-- 多选 -->
        <view v-if="type === 'checkbox' && isArray(selectList)" id="ht-checkbox-group">
          <ht-checkbox-group v-model="selectList" cell :size="selectSize" :checked-color="checkedColor" :min="min" :max="max" @change="handleChange">
            <view v-for="item in filterColumns" :key="item[valueKey]" :id="'check' + item[valueKey]">
              <ht-checkbox :modelValue="item[valueKey]" :disabled="item.disabled">
                <block v-if="filterable && filterVal">
                  <block v-for="text in item[labelKey]" :key="text.label">
                    <text v-if="text.type === 'active'" class="ht-select-picker__text-active">{{ text.label }}</text>
        ...
        <!-- 单选 -->
        <view v-if="type === 'radio' && !isArray(selectList)" id="ht-radio-group">
          <ht-radio-group v-model="selectList" cell :size="selectSize" :checked-color="checkedColor" @change="handleChange">
            <view v-for="(item, index) in filterColumns" :key="index" :id="'radio' + item[valueKey]">
              <ht-radio :value="item[valueKey]" :disabled="item.disabled">
                <block v-if="filterable && filterVal">
                  <block v-for="text in item[labelKey]" :key="text.label">
                    <text :class="`${text.type === 'active' ? 'ht-select-picker__text-active' : ''}`">{{ text.label }}</text>
                  </block>
                </block>
                <block v-else>
                  {{ item[labelKey] }}
        ...
        <view v-if="loading" class="ht-select-picker__loading" @touchmove="noop">
          <ht-loading :color="loadingColor" />
        </view>
      </scroll-view>
      <!-- 确认按钮 -->
      <view v-if="showConfirm" class="ht-select-picker__footer">
        <ht-button block size="large" @click="onConfirm" :disabled="loading">{{ confirmButtonText || translate('confirm') }}</ht-button>
      </view>
    </ht-action-sheet>
  </view>
</template>
...
import wdActionSheet from '../ht-action-sheet/ht-action-sheet.vue'
import wdCheckbox from '../ht-checkbox/ht-checkbox.vue'
import wdCheckboxGroup from '../ht-checkbox-group/ht-checkbox-group.vue'
import wdRadio from '../ht-radio/ht-radio.vue'
import wdRadioGroup from '../ht-radio-group/ht-radio-group.vue'
...
import { FORM_KEY, type FormItemRule } from '../ht-form/types'
...
async function setScrollIntoView() {
  ...
  if (isDef(selectList.value) && selectList.value !== '' && !isArray(selectList.value)) {
    ...
    wraperSelector = '#ht-checkbox-group'
  }
  if (wraperSelector) {
    await pause(2000 / 30)
    Promise.all([getRect('.ht-select-picker__wrapper', false, proxy), getRect(wraperSelector, false, proxy), ...selectorPromise]).then((res) => {
