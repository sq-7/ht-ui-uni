<template>
  <view :class="rootClass">
    <!-- 前缀插槽 -->
    <slot name="prefix">
      <ht-text :type="props.type" :color="props.color" :size="`${props.fontSize * 0.7}px`" :text="props.prefix"></ht-text>
    </slot>
    <!-- 默认文本插槽 -->
    <slot>
      <ht-text :type="props.type" :color="props.color" :size="`${props.fontSize}px`" :text="timeText"></ht-text>
    </slot>
    <!-- 后缀插槽 -->
    <slot name="suffix">
      <ht-text :type="props.type" :color="props.color" :size="`${props.fontSize * 0.7}px`" :text="props.suffix"></ht-text>
    </slot>
  </view>
</template>
...
import wdText from '../ht-text/ht-text.vue'
import { computed, watch, onMounted } from 'vue'
import { countToProps } from './types'
import { easingFn, isNumber } from '../common/util'
import { useCountDown } from '../composables/useCountDown'
import type { CountDownExpose } from '../ht-count-down/types'
...
const rootClass = computed(() => {
  return `ht-count-to ${props.customClass}`
})
