<template>
  <view :class="['ht-upload', customClass]" :style="customStyle">
    <!-- 预览列表 -->
    <view :class="['ht-upload__preview', customPreviewClass]" v-for="(file, index) in uploadFiles" :key="index">
      <!-- 成功时展示图片 -->
      <view class="ht-upload__status-content">
        <image v-if="isImage(file)" :src="file.url" :mode="imageMode" class="ht-upload__picture" @click="onPreviewImage(file)" />
        <template v-else-if="isVideo(file)">
          <view class="ht-upload__video" v-if="file.thumb" @click="onPreviewVideo(file)">
            <image :src="file.thumb" :mode="imageMode" class="ht-upload__picture" />
            <ht-icon name="play-circle-filled" custom-class="ht-upload__video-paly"></ht-icon>
          </view>
          <view v-else class="ht-upload__video" @click="onPreviewVideo(file)">
            <!-- #ifdef APP-PLUS || MP-DINGTALK -->
            <ht-icon custom-class="ht-upload__video-icon" name="video"></ht-icon>
            <!-- #endif -->
            <!-- #ifndef APP-PLUS -->
            <!-- #ifndef MP-DINGTALK -->
            <video
        ...
        <view v-else class="ht-upload__file" @click="onPreviewFile(file)">
          <ht-icon name="file" custom-class="ht-upload__file-icon"></ht-icon>
          <view class="ht-upload__file-name">{{ file.name || file.url }}</view>
        </view>
      </view>

      <view v-if="file[props.statusKey] !== 'success'" class="ht-upload__mask ht-upload__status-content">
        <!-- loading时展示loading图标和进度 -->
        <view v-if="file[props.statusKey] === 'loading'" class="ht-upload__status-content">
          <ht-loading :type="loadingType" :size="loadingSize" :color="loadingColor" />
          <text class="ht-upload__progress-txt">{{ file.percent }}%</text>
        </view>
        <!-- 失败时展示失败图标以及失败信息 -->
        <view v-if="file[props.statusKey] === 'fail'" class="ht-upload__status-content">
          <ht-icon name="close-outline" custom-class="ht-upload__icon"></ht-icon>
          <text class="ht-upload__progress-txt">{{ file.error || translate('error') }}</text>
        </view>
      </view>
      <!-- 上传状态为上传中时不展示移除按钮 -->
      <ht-icon
    ...
    <block v-if="showUpload">
      <view :class="['ht-upload__evoke-slot', customEvokeClass]" v-if="$slots.default" @click="handleChoose">
        <slot></slot>
      </view>
      <!-- 唤起项 -->
      <view v-else @click="handleChoose" :class="['ht-upload__evoke', disabled ? 'is-disabled' : '', customEvokeClass]">
        <!-- 唤起项图标 -->
        <ht-icon class="ht-upload__evoke-icon" name="fill-camera"></ht-icon>
        <!-- 有限制个数时确认是否展示限制个数 -->
        <view v-if="limit && showLimitNum" class="ht-upload__evoke-num">（{{ uploadFiles.length }}/{{ limit }}）</view>
      </view>
    </block>
  </view>
  <ht-video-preview ref="videoPreview"></ht-video-preview>
</template>
...
import wdVideoPreview from '../ht-video-preview/ht-video-preview.vue'
...
import type { VideoPreviewInstance } from '../ht-video-preview/types'
...
watch(
  ...
  (val) => {
    if (val && val < uploadFiles.value.length) {
      console.error('[ht-design]Error: props limit must less than fileList.length')
    }
  },
  {
    deep: true,
