<template>
  <view :style="`${rootStyle};display: inline-block;`">
    <view :class="`ht-sticky ${customClass}`" :style="stickyStyle" :id="styckyId">
      <view class="ht-sticky__container" :style="containerStyle">
        <ht-resize @resize="handleResize" custom-style="display: inline-block;">
          <slot />
        </ht-resize>
      </view>
    </view>
...
import wdResize from '../ht-resize/ht-resize.vue'
...
import { STICKY_BOX_KEY } from '../ht-sticky-box/types'

const props = defineProps(stickyProps)
const styckyId = ref<string>(`ht-sticky${uuid()}`)
...
function handleRelativeTo({ boundingClientRect }: any) {
  // sticky 高度大于或等于 ht-sticky-box，使用 ht-sticky-box 无任何意义
