<template>
  <view :class="`ht-table ${border ? 'is-border' : ''} ${customClass}`" :style="tableStyle">
    <template v-if="fixedHeader">
      <scroll-view
        ...
        :scroll-x="true"
        class="ht-table__header"
        @scroll="scroll"
        v-if="showHeader"
      >
        <view id="table-header" class="ht-table__content ht-table__content--header" :style="realWidthStyle">
          <view
            :class="`ht-table__cell ${border ? 'is-border' : ''} ${column.fixed ? 'is-fixed' : ''} ${stripe ? 'is-stripe' : ''} is-${column.align} ${
          ...
          >
            ...
            />
            <text v-else :class="`ht-table__value ${ellipsis ? 'is-ellipsis' : ''}`">{{ column.label }}</text>
          </view>
        </view>
      </scroll-view>
      <scroll-view
        class="ht-table__body"
        :style="bodyStyle"
        :enable-flex="true"
        :throttle="false"
        :scroll-x="true"
        ...
        :scrollLeft="state.scrollLeft"
      >
        <view id="table-body" class="ht-table__content" :style="realWidthStyle">
          <ht-table-col
            v-if="index !== false"
    ...
    <template v-else>
      <scroll-view class="ht-table__wrapper" :enable-flex="true" :throttle="false" :scroll-x="true" @scroll="scroll" :scrollLeft="state.scrollLeft">
        <view class="ht-table__inner" :style="realWidthStyle">
          <!-- 表头部分 -->
          <view v-if="showHeader" class="ht-table__header-row">
            <view
              v-for="(column, index) in children"
              :key="index"
              :class="`ht-table__cell ${border ? 'is-border' : ''} ${column.fixed ? 'is-fixed' : ''} ${stripe ? 'is-stripe' : ''} is-${
                column.align
              } ${getIsLastFixed(column) && state.scrollLeft ? 'is-shadow' : ''}`"
              :style="getCellStyle(index)"
            >
          ...
          <!-- 表格内容部分 -->
          <view class="ht-table__content" :style="bodyStyle">
            <ht-table-col
              v-if="index !== false"
              :prop="indexColumn.prop"
              :label="indexColumn.label"
...
import wdTableCol from '../ht-table-col/ht-table-col.vue'
import wdSortButton from '../ht-sort-button/ht-sort-button.vue'
...
import type { SortDirection, TableColumn, TableColumnInstance, TableColumnProps } from '../ht-table-col/types'
