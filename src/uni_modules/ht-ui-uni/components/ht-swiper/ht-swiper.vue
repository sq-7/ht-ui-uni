<template>
  <view :class="`ht-swiper ${customClass}`" :style="customStyle">
    <!-- #ifdef MP-WEIXIN -->
    <scroll-view scroll-x scroll-y style="width: 100%; height: 100%">
      ...
      >
        <swiper-item v-for="(item, index) in list" :key="index" class="ht-swiper__item">
          <video
            v-if="isVideo(item)"
            :id="`video-${index}-${uid}`"
            :style="{ height: addUnit(height) }"
            ...
            :class="`ht-swiper__video ${customItemClass} ${getCustomItemClass(currentValue, index, list)}`"
          ...
          <image
            v-else
            :src="isObj(item) ? item[valueKey] : item"
            :class="`ht-swiper__image ${customImageClass} ${customItemClass} ${getCustomItemClass(currentValue, index, list)}`"
            :style="{ height: addUnit(height) }"
            :mode="imageMode"
            @click="handleClick(index, item)"
          />

          <text v-if="isObj(item) && item[textKey]" :class="`ht-swiper__text ${customTextClass}`" :style="customTextStyle">{{ item[textKey] }}</text>
        </swiper-item>
      </swiper>
      <!-- #ifdef MP-WEIXIN -->
    </scroll-view>
...
import wdSwiperNav from '../ht-swiper-nav/ht-swiper-nav.vue'
...
import type { SwiperNavProps } from '../ht-swiper-nav/types'
