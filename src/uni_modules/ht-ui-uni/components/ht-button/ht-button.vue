<template>
  <button
    :id="buttonId"
    :hover-class="`${disabled || loading ? '' : 'ht-button--active'}`"
    :style="customStyle"
    :class="[
      'ht-button',
      'is-' + type,
  ...
  >
    <view class="ht-button__content">
      <view v-if="loading" class="ht-button__loading">
        <view class="ht-button__loading-svg" :style="loadingStyle"></view>
      </view>
      <ht-icon v-else-if="icon" custom-class="ht-button__icon" :name="icon" :classPrefix="classPrefix"></ht-icon>
      <view class="ht-button__text"><slot /></view>
    </view>
  </button>
</template>
