<template>
  <view :style="customStyle" :class="`ht-drop-menu ${customClass}`" @click.stop.prevent="noop" :id="dropMenuId">
    <ht-overlay :show="overlayVisible" :duration="duration" :z-index="12" :custom-style="modalStyle" @click="handleClickOverlay" @touchmove="noop" />

    <!-- #ifdef MP-DINGTALK -->
    <view :id="dropMenuId">
      <!-- #endif -->
      <view class="ht-drop-menu__list">
        <view
          v-for="(child, index) in children"
          :key="index"
          @click="toggle(child)"
          :class="`ht-drop-menu__item ${child.disabled ? 'is-disabled' : ''} ${child.$.exposed!.getShowPop() ? 'is-active' : ''}`"
        >
          <view class="ht-drop-menu__item-title">
            <view class="ht-drop-menu__item-title-text">{{ getDisplayTitle(child) }}</view>
            <ht-icon :name="child.icon" :size="child.iconSize" custom-class="ht-drop-menu__arrow" />
          </view>
        </view>
      </view>
      <slot />
...
watch(
  ...
  (newValue) => {
    if (!['up', 'down'].includes(newValue)) {
      ...
      console.error("[wot design] warning(ht-drop-menu): direction must be 'up' or 'down'")
    }
  },
...
function getDisplayTitle(child: any) {
  ...
  console.error('[ht-design] warning(ht-drop-menu-item): no value is matched in the options option.')
}
