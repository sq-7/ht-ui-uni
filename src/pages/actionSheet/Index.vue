  ...
  <page-wraper>
    ...
    <view>
      <demo-block :title="$t('jiBenYongFa')">
        <ht-button @click="showActions1">{{ $t('danChuCaiDan') }}</ht-button>
        <ht-action-sheet v-model="show" :actions="actions" />
      </demo-block>
      <demo-block :title="$t('xuanXiangZhuangTai')">
        <ht-button @click="showActions2">{{ $t('danChuCaiDan') }}</ht-button>
      </demo-block>
      <demo-block :title="$t('quXiaoAnNiu')">
        <ht-button @click="showActions3">{{ $t('danChuCaiDan') }}</ht-button>
        <ht-action-sheet v-model="show1" :actions="actions" :cancel-text="$t('qu-xiao')" @close="close1" />
      </demo-block>
      <demo-block :title="$t('ziDingYiMianBanDanHang')">
        <ht-button @click="showActions4">{{ $t('danChuCaiDan') }}</ht-button>
        <ht-action-sheet v-model="show2" :panels="panels" :cancel-text="$t('qu-xiao')" @close="close2" @select="select" />
      </demo-block>
      <demo-block :title="$t('ziDingYiMianBanDuoHang')">
        <ht-button @click="showActions5">{{ $t('danChuCaiDan') }}</ht-button>
        <ht-action-sheet v-model="show3" :panels="panels" :cancel-text="$t('qu-xiao')" @close="close3" @select="select1" />
      </demo-block>
      <demo-block :title="$t('biaoTi-0')">
        <ht-button @click="showActions6">{{ $t('danChuCaiDan') }}</ht-button>
      </demo-block>
      <ht-action-sheet v-model="show4" :title="$t('biaoTi-0')" @close="close4" :cancelText="cancelText">
