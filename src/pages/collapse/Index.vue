  ...
  <view class="collapse">
    <page-wraper>
      <demo-block title="toggleAll">
        <ht-button custom-class="custom-button" type="info" @click="collapse?.toggleAll()">{{ $t('quan-bu-qie-huan') }}</ht-button>
        <ht-button custom-class="custom-button" type="success" @click="collapse?.toggleAll(true)">{{ $t('quan-bu-zhan-kai') }}</ht-button>
        <ht-button custom-class="custom-button" type="primary" @click="collapse?.toggleAll(false)">{{ $t('quan-bu-shou-qi') }}</ht-button>
        <ht-button custom-class="custom-button" type="warning" @click="collapse?.toggleAll({ skipDisabled: true })">
          {{ $t('quan-bu-qie-huan-tiao-guo-jin-yong') }}
        </ht-button>
        <ht-button custom-class="custom-button" type="error" @click="collapse?.toggleAll({ expanded: true, skipDisabled: true })">
          {{ $t('quan-bu-xuan-zhong-tiao-guo-jin-yong') }}
        </ht-button>
      </demo-block>
      <demo-block :title="$t('ji-chu-yong-fa')" transparent>
        <ht-collapse ref="collapse" v-model="value1" @change="handleChange1">
          <ht-collapse-item
      ...
      <demo-block :title="$t('zi-ding-yi-title')" transparent>
        <ht-collapse v-model="value7">
          <ht-collapse-item name="item1">
          ...
          <ht-collapse-item name="item2" disabled>
      ...
      <demo-block :title="$t('shou-feng-qin')" transparent>
        <ht-collapse v-model="value2" :accordion="accordion" @change="handleChange2">
          <ht-collapse-item :title="$t('biao-qian-1')" name="item1">{{ $t('zhe-shi-yi-tiao-jian-dan-de-shi-li-wen-zi') }}</ht-collapse-item>
          <ht-collapse-item :title="$t('biao-qian-2')" name="item2">
            {{
              $t(
          ...
          <ht-collapse-item :title="$t('biao-qian-3')" name="item3">{{ $t('zhe-shi-yi-tiao-jian-dan-de-shi-li-wen-zi-0') }}</ht-collapse-item>
        </ht-collapse>
      </demo-block>
      <demo-block :title="$t('jinYong')" transparent>
        <ht-collapse v-model="value3" @change="handleChange3">
          <ht-collapse-item :title="$t('biao-qian-1-0')" name="item1">
            {{
              $t(
          ...
          <ht-collapse-item :title="$t('biao-qian-2-0')" name="item2" disabled>
            {{ $t('zhe-shi-yi-tiao-jian-dan-de-shi-li-wen-zi-1') }}
          </ht-collapse-item>
          <ht-collapse-item :title="$t('biao-qian-3-0')" name="item3">{{ $t('zhe-shi-yi-tiao-jian-dan-de-shi-li-wen-zi-2') }}</ht-collapse-item>
        </ht-collapse>
      </demo-block>

      <demo-block :title="$t('qian-tao')" transparent>
        <ht-collapse v-model="collapseRoot" @change="handleChange1">
          <ht-collapse-item
      ...
      <demo-block :title="$t('cha-kan-geng-duo')" transparent>
        <ht-collapse viewmore v-model="value4" @change="handleChange4">
          {{
            $t(
      ...
      <demo-block :title="$t('cha-kan-geng-duo-hang-shu-xian-shi-she-zhi')" transparent>
        <ht-collapse viewmore v-model="value5" @change="handleChange5" :line-num="3">
          {{
            $t(
      ...
      <demo-block :title="$t('cha-kan-geng-duo-ju-ming-cha-cao')" transparent>
        <ht-collapse viewmore v-model="value6" @change="handleChange6" use-more-slot custom-more-slot-class="more-slot">
          {{
            $t(
...
import type { CollapseInstance } from '@/uni_modules/ht-ui-uni/components/ht-collapse/types'
import { ref } from 'vue'
