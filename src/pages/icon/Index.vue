    ...
    <page-wraper>
      <view class="icon">
        <view style="position: sticky; top: 0; z-index: 2">
          <ht-search hide-cancel :placeholder="$t('cha-zhao-tu-biao')" light v-model="keyword" @search="handleSearch" @clear="handleClear" />
        </view>
        <view class="icon-list">
          <view v-for="(icon, index) in showIcons" :key="index" class="icon-item" @click="handleClick(icon)">
            <view><ht-icon :name="icon" size="22px" custom-class="icon-item-class" /></view>
            <view class="icon-item-name">{{ icon }}</view>
          </view>
          <ht-status-tip v-if="!showIcons.length" image="search" :tip="$t('dang-qian-wu-xiang-guan-tu-biao')" />
        </view>
      </view>
    </page-wraper>
  </view>
...
function handleClick(icon: string) {
  ...
  uni.setClipboardData({
    data: `<ht-icon name="${icon}" size="22px"></ht-icon>`,
    showToast: false,
    success: () => {
      showNotify({
        type: 'success',
        duration: 1500,
        message: t('fu-zhi-cheng-gong') + `<ht-icon name="${icon}" size="22px"></ht-icon>`
      })
    }
  })
