  ...
  <page-wraper>
    ...
    <view>
      <demo-block :title="$t('jiBenYongFa')">
        <ht-switch v-model="checked1" @change="handleChange1" />
      </demo-block>
      <demo-block :title="$t('xiu-gai-zhi-activevalue-inactivevalue')">
        <view style="margin-bottom: 10px">{{ checked2 }}</view>
        <ht-switch v-model="checked2" active-value="沃特" inactive-value="商家后台" @change="handleChange2" />
      </demo-block>
      <demo-block :title="$t('zi-ding-yi-yan-se-activecolor-inactivecolor')">
        <ht-switch v-model="checked3" active-color="#13ce66" inactive-color="#f00" @change="handleChange3" />
      </demo-block>
      <demo-block :title="$t('zi-ding-yi-da-xiao')">
        <ht-switch v-model="checked4" :size="24" @change="handleChange4" />
      </demo-block>
      <demo-block :title="$t('xuan-zhong-jin-yong')">
        <ht-switch v-model="checked5" disabled />
      </demo-block>
      <demo-block :title="$t('fei-xuan-zhong-jin-yong')">
        <ht-switch v-model="checked6" disabled />
      </demo-block>
      <demo-block :title="$t('beforechange-xiu-gai-qian-gou-zi-han-shu')">
        <ht-switch v-model="checked7" :before-change="beforeChange" @change="handleChange5" />
      </demo-block>
    </view>
  </page-wraper>
</template>
...
import type { SwitchBeforeChange } from '@/uni_modules/ht-ui-uni/components/ht-switch/types'
import { ref } from 'vue'
