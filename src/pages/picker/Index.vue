  ...
  <page-wraper>
    ...
    <demo-block transparent>
      <ht-cell-group border>
        <ht-picker :label="$t('dan-lie-xuan-xiang')" v-model="value0" :columns="columns0" />
        <ht-picker :label="$t('jinYong')" disabled v-model="value1" :columns="columns1" />
        <ht-picker :label="$t('zhi-du')" readonly v-model="value2" :columns="columns2" />
        <ht-picker label="loading" v-model="value3" loading :columns="columns3" />
        <ht-picker :label="$t('duo-lie')" v-model="value4" :columns="columns4" />
        <ht-picker :label="$t('duo-ji-lian-dong')" v-model="value5" :columns="columns5" :column-change="onChangeDistrict" />
        <ht-picker :label="$t('fen-ge-fu')" v-model="value6" :columns="columns6" :display-format="displayFormat" />
        <ht-picker :label="$t('biaoTi-0')" v-model="value9" :columns="columns7" :title="$t('wen-an-biao-ti')" />
        <ht-picker label="before-confirm" :columns="columns0" v-model="value7" :before-confirm="beforeConfirm" />
        <ht-picker :label="$t('cuo-wu')" v-model="value10" error :columns="columns0" />
        <ht-picker :label="$t('bi-tian')" v-model="value11" :columns="columns0" required />
        <ht-picker :label="$t('ke-qing-kong')" :clearable="true" v-model="value15" :columns="columns5" :column-change="onChangeDistrict" />
      </ht-cell-group>
    </demo-block>
    <demo-block :title="$t('label-bu-chuan-0')" transparent>
      <ht-picker :columns="columns0" v-model="value12" />
    </demo-block>
    <demo-block :title="$t('da-xiao')" transparent>
      <ht-picker :label="$t('dan-lie-xuan-xiang')" v-model="value13" size="large" :columns="columns0" />
    </demo-block>
    <demo-block :title="$t('zhi-kao-you-xian-shi')" transparent>
      <ht-picker :label="$t('dan-lie-xuan-xiang')" v-model="value14" align-right :columns="columns0" />
    </demo-block>
    <demo-block :title="$t('mo-ren-cha-cao')" transparent>
      <view class="default-slot">
        ...
        <ht-picker :columns="columns0" v-model="value8" use-default-slot @confirm="handleConfirm">
          <ht-button>{{ $t('cha-cao-huan-qi') }}</ht-button>
        </ht-picker>
      </view>
    </demo-block>
  </page-wraper>
...
import type { ColumnItem, PickerViewColumnChange } from '@/uni_modules/ht-ui-uni/components/ht-picker-view/types'
import type { PickerBeforeConfirm, PickerDisplayFormat } from '@/uni_modules/ht-ui-uni/components/ht-picker/types'
import { ref } from 'vue'
