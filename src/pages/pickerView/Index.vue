<template>
  <page-wraper>
    <demo-block :title="$t('ji-ben-yong-fa-shu-zhi-value1') + value1">
      <ht-picker-view v-model="value1" :columns="columns1" />
    </demo-block>

    <demo-block :title="$t('jin-yong-xuan-xiang-shu-zhi-value2') + value2">
      <ht-picker-view v-model="value2" :columns="columns2" />
    </demo-block>

    <demo-block :title="$t('li-ji-chu-fa-change-shu-zhi-value6') + value6">
      <ht-picker-view v-model="value6" :immediate-change="true" :columns="columns2" />
    </demo-block>

    <demo-block :title="$t('jia-zai-zhong-shu-zhi-value3') + value3">
      <ht-picker-view v-model="value3" :columns="columns3" loading />
    </demo-block>

    <demo-block :title="$t('duo-lie-shu-zhi-value4') + `[${value4}]`">
      <ht-picker-view v-model="value4" :columns="columns4" />
    </demo-block>

    <demo-block :title="$t('duo-ji-lian-dong-shu-zhi-value5') + `[${value5}]`">
      <ht-picker-view v-model="value5" :columns="columns5" :column-change="onChangeDistrict" />
    </demo-block>
  </page-wraper>
</template>
<script lang="ts" setup>
import type { PickerViewColumnChange } from '@/uni_modules/ht-ui-uni/components/ht-picker-view/types'
import { ref } from 'vue'
