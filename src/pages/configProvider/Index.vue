<template>
  <page-wraper show-dark-mode>
    <ht-message-box />
    <ht-toast />
    <ht-action-sheet v-model="showAction" :actions="actions" />

    <form @submit="formSubmit">
      <ht-cell-group custom-class="group" :title="$t('ji-chu-xin-xi')" border>
        <ht-input
        ...
        />
        <ht-picker :label="$t('you-hui-fang-shi')" label-width="100px" name="promotion" align-right v-model="promotion" :columns="promotionlist" />
        <ht-cell :title="$t('quan-mian-e')" required title-width="100px" custom-value-class="cell-left">
      ...
      <ht-cell-group custom-class="group" :title="$t('shi-jian-he-di-zhi')" border>
        <ht-datetime-picker :label="$t('shi-jian')" label-width="100px" name="date" v-model="date" @confirm="handleDate" />
        <ht-col-picker
          :label="$t('di-zhi')"
          label-width="100px"
          name="address"
      ...
      <ht-cell-group custom-class="group" :title="$t('qi-ta-xin-xi')" border>
        <ht-input
        ...
        />
        <ht-cell :title="$t('fa-huo-shu-liang')" center>
          <ht-input-number v-model="count" name="count" @change="handleCount" />
        </ht-cell>
        <ht-cell :title="$t('zhe-li-xian-shi-de-shi-duo-wen-zi-biao-ti-bao-han-fei-chang-de-wen-zi')" title-width="240px" center>
          <ht-switch v-model="switchVal" name="switchVal" @change="handleSwitch" />
        </ht-cell>
        <ht-input
      ...
      <view class="tip">
        <ht-checkbox v-model="read" name="read" @change="handleRead" custom-label-class="label-class">
          {{ $t('yi-yue-du-bing-tong-yi') }}
          <text style="color: #4d80f0">{{ $t('ba-la-ba-la-ba-la-xie-yi') }}</text>
        </ht-checkbox>
      </view>
      <view class="footer">
        <ht-button block round size="large">{{ $t('ti-jiao') }}</ht-button>
      </view>
    </form>
  </page-wraper>
</template>
...
import type { ColPickerColumnChangeOption } from '@/uni_modules/ht-ui-uni/components/ht-col-picker/types'
import { ref } from 'vue'
