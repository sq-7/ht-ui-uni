    ...
    <view class="page">
      <view class="page__hd">
        ...
        <view class="page__desc">
          {{
            $t(
              'ht-ui-uni-shi-yi-ge-ji-yu-vue3ts-kai-fa-de-uniapp-zu-jian-ku-ti-gong-70-gao-zhi-liang-zu-jian-zhi-chi-an-hei-mo-shi-guo-ji-hua-he-zi-ding-yi-zhu-ti'
            )
          }}
        </view>
      </view>
      <view class="page__bd">
        <block v-for="(item, index) in list" :key="index">
          <view class="kind-list__item">
            <view :id="item.id" class="ht-flex kind-list__item-hd" @click="kindToggle(item.id)">
              <view class="ht-flex__item title">{{ item.name }}</view>
              <image class="kind-list__img" :src="item.icon"></image>
            </view>
            <view :class="['kind-list__item-bd', openState[item.id] ? 'kind-list__item-bd_show' : '']">
              <view :class="['ht-cells', openState[item.id] ? 'ht-cells_show' : '']">
                <ht-cell
...
.ht-tool-right-line-angle::after {
  content: ' ';
  display: inline-block;
  height: 8px;
  width: 8px;
