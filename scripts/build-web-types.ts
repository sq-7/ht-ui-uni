...
const reComponentName: ReComponentName = (title: string) => {
  return `ht-${hyphenate(removeHtmlTagsAndNonEnglish(title)).replace(/[ ]+/g, '-')}`
}

// 重新定义文档 URL 的函数
const reDocUrl: ReDocUrl = (fileName, header) => {
  const docs = 'https://ht-ui-uni.cn/component/'
...
generateWebTypes({
  ...
  version,
  entry,
  outDir: path.resolve(__dirname, '../src/uni_modules/ht-ui-uni'),
  reComponentName,
  reDocUrl,
  reWebTypesSource,
  reWebTypesType,
